...
const CAFETERIAS: Cafeteria[] = [
  ...
  { id: "wonka", name: "Wonka", short: "Wonka", x: 412, y: 480, zone: "Frente al Ed. L", cat: "snack", building: "—" },
  {
    ...
    building: "Edificio A",
    subItems: [
      { label: "Café Sabor", slug: "cafe-sabor" },
      { label: "Isabela", slug: "isabella" },
      { label: "Anthony's Chef", slug: "anthonys-chef" },
      { label: "Bristo Central", slug: "bristo-central" },
    ],
  },
  { id: "bristo-g", name: "Bristo G", short: "Bristo G", x: 662, y: 580, zone: "Edificio G", cat: "rest", building: "Edificio G" },
  { id: "bristo-f", name: "The Snack Ed. F", short: "Snack F", x: 450, y: 680, zone: "Edificio F", cat: "snack", building: "Edificio F" },
];

/* Mapping from map pin ID to app route slug */
const SLUG_MAP: Record<string, string> = {
  qbano: "qbano",
  quindio: "cafe-quindio",
  ventolini: "ventolini",
  "snack-bu": "the-snack-bienest",
  "cafe-sol": "cafe-del-sol",
  wonka: "wonka",
  "bristo-g": "bristo-g",
  "bristo-f": "the-snack-f",
};

/* Route slug → queue-data key (slugs now match queue keys directly) */
function slugToQueueKey(slug: string): string {
  return slug;
}

const CAT_META: Record<CafCat, { label: string; bg: string; color: string; grad: string }> = {
...
const QUEUE_PIN_COLOR: Record<QueueLevel, string> = {
  low:  "#34D399",
  mid:  "#FBBF24",
  high: "#F87171",
};

// Maps map-pin ID → queue-data key (or array of keys for multi-vendor pins)
const MAP_TO_QUEUE: Record<string, string | string[]> = {
  qbano:      "qbano",
  quindio:    "cafe-quindio",
  ventolini:  "ventolini",
  "snack-bu": "the-snack-bienest",
  ...
  "bristo-g": "bristo-g",
  "bristo-f": "the-snack-f",
  central:    ["cafe-sabor", "isabella", "anthonys-chef", "bristo-central"],
};

const LEVEL_RANK: Record<QueueLevel, number> = { low: 0, mid: 1, high: 2 };
const RANK_LEVEL: QueueLevel[] = ["low", "mid", "high"];

function pinQueueLevel(cafId: string, allQueue: Record<string, { level: QueueLevel }>): QueueLevel {
  const key = MAP_TO_QUEUE[cafId];
  if (!key) return "low";
  if (Array.isArray(key)) {
    const max = key.reduce((acc, k) => {
      const rank = LEVEL_RANK[allQueue[k]?.level ?? "low"];
      return Math.max(acc, rank);
    }, 0);
    return RANK_LEVEL[max];
  }
  return allQueue[key]?.level ?? "low";
}
...
function UtensilsIcon({ size = 14, color = T.accent }: { size?: number; color?: string }) {
  return (
...
function CoffeeIcon({ size = 14, color = T.cafeBadgeTxt }: { size?: number; color?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
...
function SnackIcon({ size = 14, color = T.snackBadgeTxt }: { size?: number; color?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
...
function NavIcon({ size = 10, color = T.textMuted }: { size?: number; color?: string }) {
  return (
...
function getIcon(cat: CafCat, size: number) {
...
/* ══════════════════════════ HELPERS ══════════════════════════ */
const cafHasBuilding = (bid: string) =>
  CAFETERIAS.some(
    (c) =>
      (bid === "I" && c.id === "snack-bu") ||
      (bid === "G" && c.id === "bristo-g") ||
...
function getTouchDist(t1: { clientX: number; clientY: number }, t2: { clientX: number; clientY: number }): number {
...
/* ══════════════════════════ MAIN COMPONENT ══════════════════════════ */
interface IcesiCampusMapProps {
  onNavigate?: (slug: string) => void;
  embedded?: boolean;
}

export default function IcesiCampusMap({ onNavigate: onNavigateProp, embedded = false }: IcesiCampusMapProps) {
  const navigate = useNavigate();
  const allQueueData = useAllQueueData();
  const [selected, setSelected] = useState<Cafeteria | null>(null);
  const [centralOpen, setCentralOpen] = useState(false);
  const [sheetOpen, setSheetOpen] = useState(false);
  const [hoveredCaf, setHoveredCaf] = useState<string | null>(null);
  const cardRefs = useRef<Record<string, HTMLDivElement | null>>({});
  const [pendingNav, setPendingNav] = useState<{ slug: string; name: string } | null>(null);
  ...
  const showConfirmBanner = useCallback((slug: string, name: string) => {
    if (dismissTimerRef.current) clearTimeout(dismissTimerRef.current);
    setSelected(null);
    setCentralOpen(false);
    setPendingNav({ slug, name });
    dismissTimerRef.current = setTimeout(() => setPendingNav(null), 6000);
  }, []);

  // ── Zoom / pan state ──
  const [mapTransform, setMapTransform] = useState({ zoom: 1, panX: 0, panY: 0 });
  const touchStateRef = useRef<{ type: 'pan' | 'pinch'; lastX: number; lastY: number; lastDist: number } | null>(null);
  ...
  const handleTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (!ts) return;
    if (e.touches.length >= 2 && ts.type === 'pinch') {
      const dist = getTouchDist(e.touches[0], e.touches[1]);
      const cx = (e.touches[0].clientX + e.touches[1].clientX) / 2;
      const cy = (e.touches[0].clientY + e.touches[1].clientY) / 2;
      const rect = mapContainerRef.current?.getBoundingClientRect();
      const pivotX = rect ? cx - rect.left : cx;
      const pivotY = rect ? cy - rect.top : cy;
      const scale = dist / ts.lastDist;
      ts.lastDist = dist;
      setMapTransform(prev => {
        const newZoom = Math.min(4, Math.max(1, prev.zoom * scale));
        if (newZoom === 1) return { zoom: 1, panX: 0, panY: 0 };
        const ratio = newZoom / prev.zoom;
        return {
          zoom: newZoom,
          panX: pivotX - (pivotX - prev.panX) * ratio,
          panY: pivotY - (pivotY - prev.panY) * ratio,
        };
      });
    } else if (e.touches.length === 1 && ts.type === 'pan') {
  ...
  const handleTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    if (e.touches.length === 0) {
      ...
      touchStateRef.current = { type: 'pan', lastX: e.touches[0].clientX, lastY: e.touches[0].clientY, lastDist: 0 };
    }
  };

  const goToMenu = (slug: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (onNavigateProp) onNavigateProp(slug);
    else navigate(`/cafeteria/${slug}`);
  };
    ...
    >
      ...
      <style>{`
        ...
        *::-webkit-scrollbar-track{background:transparent}
        .menu-btn:active{opacity:.8;transform:scale(.98)}
      `}</style>
      ...
      <div
        ...
        style={{
          ...
          overflow: "hidden",
          touchAction: "none",
          paddingBottom: !embedded ? (sheetOpen ? 324 : 144) : 0,
        }}
      ...
      >
        <svg
          ...
          style={{
            width: "100%",
            display: "block",
            transform: `translate(${mapTransform.panX}px, ${mapTransform.panY}px) scale(${mapTransform.zoom})`,
            transformOrigin: "0 0",
        ...
        >
          ...
          {CAFETERIAS.map((caf, idx) => {
            ...
            const queueLevel = pinQueueLevel(caf.id, allQueueData);
            const queueColor = QUEUE_PIN_COLOR[queueLevel];
            ...
            return (
              <g
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (embedded && !caf.subItems) {
                    showConfirmBanner(SLUG_MAP[caf.id], caf.name);
                  } else {
                    selectCaf(caf);
                  }
                }}
              ...
              >
                {/* pulse ring */}
                <circle cx={caf.x} cy={caf.y - 20} r="16" fill="none" stroke={queueColor} strokeWidth="1.5" opacity=".35">
                ...
                {isSel && (
                  <circle cx={caf.x} cy={caf.y - 20} r="22" fill="none" stroke={queueColor} strokeWidth="1" opacity=".3">
                    <animate attributeName="opacity" values=".3;.6;.3" dur="1.8s" repeatCount="indefinite" />
                  </circle>
                )}
                {/* queue-colored circle */}
                <circle
                  cx={caf.x}
                  cy={caf.y - 20}
                  r="14"
                  ...
                  strokeWidth=".8"
                />
                {/* white utensils icon (lucide paths, 24×24 → ~16px, centered) */}
                <g
                  transform={`translate(${caf.x - 8},${caf.y - 28}) scale(0.667)`}
                  stroke={T.white}
                  strokeWidth="2.2"
                  strokeLinecap="round"
                ...
                {caf.id === "central" && (
                  <g>
                    <circle cx={caf.x + 12} cy={caf.y - 32} r="6.5" fill={T.accent} stroke={T.bg} strokeWidth="1.8" />
                    <text x={caf.x + 12} y={caf.y - 29} textAnchor="middle" fontSize="7.5" fill={T.white} fontWeight="700">
                      4
                    </text>
                  </g>
                )}
      ...
      {/* ══════════════════ CONFIRMATION BANNER ══════════════════ */}
      {embedded && pendingNav && (() => {
        const qKey = slugToQueueKey(pendingNav.slug);
        const q = allQueueData[qKey];
        const isZero = !q || q.count === 0;
        const qColor = q ? QUEUE_PIN_COLOR[q.level] : QUEUE_PIN_COLOR.low;
        return (
          <>
            <div
            ...
            >
              {/* handle */}
              <div style={{ width: 36, height: 4, borderRadius: 2, background: T.textMuted, margin: "0 auto 14px", opacity: .45 }} />

              {/* name */}
              <div style={{ fontSize: 16, fontWeight: 700, color: T.text, marginBottom: 4 }}>
                Ir a {pendingNav.name}
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 13, color: T.textSec }}>
                  {isZero
                    ? "Sin espera · 0 personas en fila"
                    : `~${q!.estimatedMinutes} min de espera · ${q!.count} personas en fila`}
                </span>
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              <div
                style={{
                  ...
                  height: 36,
                  borderRadius: 10,
                  background: CAT_META[selected.cat].bg,
                  display: "flex",
                  alignItems: "center",
              ...
              >
                {getIcon(selected.cat, 16)}
              </div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: T.text, lineHeight: 1.2 }}>
                  {selected.name}
                  {selected.id === "central" && (
                    <span style={{ fontSize: 10, fontWeight: 500, color: T.accent, marginLeft: 6 }}>×4</span>
                  )}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 2 }}>
                  <NavIcon size={9} />
                  <span style={{ fontSize: 11, color: T.textSec }}>{selected.zone}</span>
                </div>
              </div>
            </div>
            {selected.subItems ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {selected.subItems.map((sub) => {
                  const subLevel = allQueueData[sub.slug]?.level ?? "low";
                  const subColor = QUEUE_PIN_COLOR[subLevel];
                  return (
                    <button
                      key={sub.slug}
                      className="menu-btn"
              ...
              <button
                className="menu-btn"
                style={{ ...menuBtnStyle, padding: "13px 20px", fontSize: 14, borderRadius: 12, marginTop: 0 }}
                onClick={(e) => goToMenu(SLUG_MAP[selected.id], e)}
              >
                Ir a esta cafetería →
              </button>
            )}
      ...
      {/* ══════════════════ BOTTOM SHEET: standalone full list ══════════════════ */}
      {!embedded && (
        <div
          style={{
            position: "absolute",
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                {(Object.entries(CAT_META) as [CafCat, (typeof CAT_META)[CafCat]][]).map(([k, v]) => (
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "0 14px 20px", WebkitOverflowScrolling: "touch" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {CAFETERIAS.map((caf) => {
                ...
                const isSel = selected?.id === caf.id;
                const slug = SLUG_MAP[caf.id];
                return (
                  <div
                    key={caf.id}
                    ref={(el) => { cardRefs.current[caf.id] = el; }}
                    className={`card ${isSel ? "on" : ""}`}
                    onClick={(e) => { e.stopPropagation(); selectCaf(caf); }}
                    style={{ padding: "12px 14px", borderRadius: 14, background: T.bg, animation: isSel ? "fadeUp .2s ease" : undefined }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 12, background: meta.bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: `1px solid ${meta.color}22` }}>
                        {getIcon(caf.cat, 18)}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13.5, fontWeight: 700, color: T.text, lineHeight: 1.2 }}>
                          {caf.name}
                          {caf.id === "central" && (
                            <span style={{ fontSize: 10, fontWeight: 500, color: T.accent, marginLeft: 6 }}>{centralOpen ? "▾" : "▸"} ×4</span>
                          )}
                        </div>
                      ...
                      <div style={{ background: meta.bg, borderRadius: 8, padding: "3px 8px", fontSize: 9, fontWeight: 600, color: meta.color, letterSpacing: ".3px", whiteSpace: "nowrap" }}>
                        {meta.label}
                      </div>
                    </div>
                    {caf.id !== "central" && isSel && slug && (
                      <button className="menu-btn" style={menuBtnStyle} onClick={(e) => goToMenu(slug, e)}>
                        Ir a esta cafetería →
                      </button>
                    )}
                    {caf.id === "central" && isSel && centralOpen && caf.subItems && (
                      <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${T.border}`, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
                        {caf.subItems.map((sub) => (
                          <div key={sub.slug} className="sub-r" style={{ padding: "8px 10px", display: "flex", flexDirection: "column", gap: 6, background: T.accentSoft, border: `1px solid ${T.accentBorder}`, borderRadius: 8 }} onClick={(e) => e.stopPropagation()}>
                            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                              <div style={{ width: 6, height: 6, borderRadius: "50%", background: T.accent, boxShadow: `0 0 4px ${T.accentGlow}`, flexShrink: 0 }} />
                              <span style={{ fontSize: 11, fontWeight: 600, color: T.text }}>{sub.label}</span>
                            </div>
                            <button className="menu-btn" style={{ ...menuBtnStyle, marginTop: 0, padding: "5px 10px", fontSize: 10 }} onClick={(e) => goToMenu(sub.slug, e)}>
                              Ir a esta cafetería →
                            </button>
                          </div>
                        ))}
