...
export default function CampusPage() {
  ...
  return (
    <div style={{ background: "var(--bg)", minHeight: "100svh", display: "flex", justifyContent: "center" }}>
      <div style={{ width: "100%", maxWidth: 430, padding: "52px 20px 96px", display: "flex", flexDirection: "column", gap: 16 }}>

        {/* ── header ── */}
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          <span style={{ fontFamily: "Satoshi", fontWeight: 400, fontSize: 18, color: "var(--text-muted)", lineHeight: 1.35 }}>
            Universidad Icesi
          </span>
          <h1 style={{ margin: 0, fontFamily: "Satoshi", fontWeight: 700, fontSize: 36, color: "var(--text-primary)", lineHeight: 1.35 }}>
            Campus
          </h1>
        </div>

        {/* ── tab toggle ── */}
        <div style={{ display: "flex", alignSelf: "center", background: "var(--card)", border: "1.5px solid var(--border)", borderRadius: 100, padding: 3 }}>
          {(["cafeterias", "mapa"] as const).map((t) => (
            <button
              ...
              style={{
                ...
                background: tab === t ? "#5454E9" : "transparent",
        ...
        {tab === "mapa" && (
          <div style={{ margin: '0 -20px' }}>
            <IcesiCampusMap embedded onNavigate={(slug) => navigate(`/cafeteria/${slug}`)} />
          </div>
        )}
        ...
        {tab === "cafeterias" && (
          <>
            {/* search */}
            <div style={{ display: "flex", alignItems: "center", gap: 9, background: "var(--card2)", border: "2px solid var(--border)", borderRadius: 12, padding: "12px 20px" }}>
              <SearchIcon />
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Buscar producto o cafetería..."
                style={{ background: "none", border: "none", outline: "none", flex: 1, fontFamily: "Satoshi", fontWeight: 400, fontSize: 16, color: "var(--text-primary)", caretColor: "var(--brand)" }}
              />
            ...
            {trimmed ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {cafeteriaResults.length === 0 && productResults.length === 0 ? (
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "48px 0", gap: 12 }}>
                    <span style={{ fontSize: 36 }}>🍽️</span>
                    <span style={{ fontFamily: "Satoshi", fontWeight: 600, fontSize: 16, color: "var(--text-muted)", textAlign: "center" }}>
                      No se encontraron resultados
                    </span>
                    <span style={{ fontFamily: "Satoshi", fontWeight: 400, fontSize: 13, color: "var(--text-label)", textAlign: "center" }}>
                    ...
                    {cafeteriaResults.length > 0 && (
                      <>
                        {productResults.length > 0 && (
                          <span style={{ fontFamily: "Satoshi", fontWeight: 600, fontSize: 12, color: "var(--text-label)", letterSpacing: "0.05em", paddingLeft: 2 }}>
                            CAFETERÍAS
                          </span>
                        )}
                        {cafeteriaResults.map((c) => {
                          const queue = allQueue[c.id] ?? { count: 0, level: "low" as const, estimatedMinutes: 1 };
                          return <CafeteriaCard key={c.id} c={c} queue={queue} onClick={() => navigate(`/cafeteria/${c.id}`)} />;
                        })}
                      </>
                    )}
                    {productResults.length > 0 && (
                      <>
                        {cafeteriaResults.length > 0 && (
                          <span style={{ fontFamily: "Satoshi", fontWeight: 600, fontSize: 12, color: "var(--text-label)", letterSpacing: "0.05em", paddingLeft: 2, marginTop: 4 }}>
                            PRODUCTOS
                          </span>
                        )}
                        {productResults.map((result) => {
                          const queue = allQueue[result.cafeteriaId] ?? { count: 0, level: "low" as const, estimatedMinutes: 1 };
                          return (
                            <SearchResultCard
                              key={`${result.cafeteriaId}-${result.item.id}`}
                              result={result}
                              queue={queue}
                              onClick={() => navigate(`/cafeteria/${result.cafeteriaId}`, { state: { focusProductId: result.item.id } })}
                            />
                          );
                        })}
                      </>
